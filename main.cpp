...
int main()
{
	...
	{
		logger.log(logLevel::INFO, __FILE__, __LINE__, "成功加载模型权重参数");

		// 为了不把训练中的相关日志信息也写入txt
		// 将此日志信息写入步骤从类成员函数中提取出来
		logger.log(logLevel::INFO, __FILE__, __LINE__, "开始进行推理");
	...
	{
		logger.log(logLevel::WARN, __FILE__, __LINE__, "未成功加载模型权重参数，准备重新训练");

		Timer t;
		example->weight_init();
		logger.log(logLevel::INFO, __FILE__, __LINE__, "模型权重参数初始化成功");

		example->weight_save("..//weight//weight1.w");
		logger.log(logLevel::INFO, __FILE__, __LINE__, "成功保持初始化的模型权重参数");

		example->backward();
		logger.log(logLevel::INFO, __FILE__, __LINE__, "反向传播训练完成");

		example->weight_save(weightPath);
		logger.log(logLevel::INFO, __FILE__, __LINE__, "模型权重参数存储完成");
