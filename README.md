...
![](./log/20260207.png)
可以发现饼状图中IO占比高达99.3%。有些不符合直觉，但也侧面说明程序内存分配部分存在极大可优化空间。查看函数详情可以发现性能问题基本出现在两行代码上。具体内容见从下图。
![](./log/20260213-1.png)
![](./log/20260213-2.png)
将权重参数从二维vector改为一维vector。由于二维vector数据存储不连续，而训练与推理过程中对权重参数访问又极为符合空间局部性原理。所以将其变为一维vector。修改后运行发现程序时间大幅减少。

但将训练时间优化到这个程度后，感觉就很难仅仅通过与内存相关的代码再有进一步的大幅提升了。性能瓶颈在权重参数中的读取与计算写入上。这个如果有一个矩阵计算库就会方便很多，但本项目目的是简单实现一个全连接神经网络，并尝试优化。没必要耗费大量时间在矩阵库上，看看后续通过多线程能不能让性能有一个可观的提升。现在CPU占用基本都在10%左右。
...
### 5.5 总结
感觉个人开发如果不是想尝试从零实现的感觉，就不必要自行造轮子。比较目前的各种库（如libtorch、opencv）都已经挺完善了。不仅在使用上，而且性能也是相对不错了。
